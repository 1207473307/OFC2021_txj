...
def data_reduction(service, G):
    ...
    node_tags[source -1] = 1
    for d in destination:
        node_tags[d -1] = 2

    for path,_,_ in path_tree:
        if path[0] not in [source] + destination:
            node_tags[path[0] -1] = 3
        if path[-1] not in [source] + destination:
            node_tags[path[0] -1] = 3
        for i in range(len(path) - 1):
            if i != 0:
                node_tags[path[i] -1] = 4
            g.add_edge(path[i]-1, path[i+1]-1)
            n_edges += 1
    G = S2VGraph(g, 0, node_tags)

    G.neighbors = [[] for i in range(len(G.g))]
    for i, j in G.g.edges():
        G.neighbors[i -1].append(j-1)
        G.neighbors[j -1].append(i-1)
    degree_list = []
    ...
    G.edge_mat = torch.LongTensor(edges).transpose(0, 1)  # 矩阵转置
